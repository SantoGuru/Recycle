  ...
  const carregarDados = async () => {
    try {
      const response = await fetch(
        "http://localhost:8080/api/dashboard/resumo",
        {
          method: "GET",
          headers: {
  ...
  return (
    <div className="mx-auto container px-8 pt-26 m-5">
          ...
          <div className="flex flex-col">
            ...
            <p className="mt-2 text-md font-bold text-gray-700">
              {valorTotal !== undefined
                ? `R$ ${converterParaReal(valorTotal)}`
                : "Erro ao carregar"}
            </p>
            <Link
              to="/stock"
      ...
      <div className="mt-5 flex flex-col md:flex-row gap-3 md:justify-end mx-auto">
        ...
        >
          <div className="flex justify-center items-center w-64 text-white p-5 bg-primary rounded-lg hover:bg-primary-hover transition-colors">
        ...
        <div className="py-7">
          ...
          >
            <EntradaMaterial
              fecharModal={fecharModalEntrada}
              atualizarEstoque={carregarDados}
            />
          </FormulariosModal>
        </div>
